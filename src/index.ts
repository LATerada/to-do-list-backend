...
app.post("/tasks", async (req: Request, res: Response) => {
  try {
    ...
    const [taskIdAlreadyExists]: TTaskDB[] | undefined[] = await db(
      "tasks"
    ).where({ id });
...
app.put("/tasks/:id", async (req: Request, res: Response) => {
  try {
    ...
    if (newtTitle !== undefined) {
      if (typeof newtTitle !== "string") {
        res.status(400);
        throw new Error("'title' deve ser string");
      }
      if (newtTitle.length < 2) {
        res.status(400);
        throw new Error("'title' deve possuir pelo menos 2 caracteres");
      }
    }
    ...
    if (newStatus !== undefined) {
      if (typeof newStatus !== "number") {
        res.status(400);
        throw new Error(
          "'status' deve ser numer (0 para incompleta ou 1 para completa)"
        );
      }
    }
